...
const STORAGE_KEY = 'clarity-openai-key';
...
async function callOpenAI({ system, userMessage, maxTokens = 60, signal }) {
  const apiKey = getOpenAIKey();
  if (!apiKey) {
    console.warn('Clarity: No OpenAI API key set.');
    return null;
  }
...
- Return ONLY the two lines starting with Q: and R: — no preamble, no explanation.`;
  ...
  const structureLine = coveredDimensions && missingDimensions
    ...
    : '';
  const userMsg = `Section: ${sectionTitle || 'Untitled'}
...
Give me the Q and R for the next ghost text focusing on the most important missing element.`;
  ...
  return callOpenAI({ system, userMessage: userMsg, maxTokens: 120, signal });
}
...
- Return ONLY the example text, no preamble.`;
  ...
  const userMsg = `Product context: ${prefaceContext || 'A B2B SaaS product'}
...
Generate a filled-in example.`;
  ...
  return callOpenAI({ system, userMessage: userMsg, maxTokens: 200, signal });
}
...
- Format: return only the observations as a numbered list. No preamble. No closing statement. Maximum 3 items.`;
  ...
  const userMsg = `Section title: ${sectionTitle}
